...
--nonsymmetric case
                      ...
                      (str3, b3, btemp3, top3, dones3) = foldIterate (process circ2 outList) inps2 (str, b, b2, top, dones2) `debug` ("after removed: "++ show inps2) -- this needs to be inps2
                  in
                   case (circ2!!bestInd) of 
                     Fun f _ li ->  
                         if (willAppears!!bestInd || (and dones3)) --if it will appear again, or all done, put on top of stack.
                          then
                              let
                                  top4 = top3 + 1 - (length li) --or length inps
                                  b4 = b3 |> Data.Bimap.insert bestInd (top4)
...
process :: Circuit Int -> [[Int]] -> Int -> (String, BI, BI, Int, [Bool]) -> (String, BI, BI, Int, [Bool])
